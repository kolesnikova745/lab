...
int N = 35, M = 25; //// çàäàíèå ðàçìåðîâ ïîëÿ
int scale = 25;
int w = scale*N;
int h = scale*M;

int direction = 2, size = 4; //// íà÷àëüíîå íàïðàâëåíèå äâèæåíèÿ, íà÷àëüíûé ðàçìåð çìåéêè
...
void Rules::game() {
	if ((life == true) && (start == true)) {
		for (int i = size; i > 0; --i)       //// àëãîðèòì ïåðåäâèæåíèÿ çìåéêè
		{
		...
		for (int i = 1; i < N; i++)                       //// ïðîâåðêà íà ñòîëêíîâåíèå ñî ñòåíàìè
		...
		if (life == true) {
			if ((snake[0].x == eda.x) && (snake[0].y == eda.y)) { //// óñëîâèå ðîñòà çìåéêè
				size++;
				spawn();
			}
		}
...
void Rules::spawn() {              //// ãåíåðàöèÿ åäû
	if (rules.start) {
		eda.x = rand() % N;
		eda.y = rand() % M;
		for (int i = 1; i < size; i++) {
...
bool startgame() {
	...
	Clock clock;
	float timer = 0, delay = 0.06; //// ñêîðîñòü èãðû
	...
	snake[0].x = 5;         //// íà÷àëüíîå ïîëîæåíèå çìåéêè
	snake[0].y = M / 2;
	...
	while (window.isOpen())
	{
		...
		if ((!rules.life) && (rules.start)) {                //// ðåñòàðò èãðû
		...
		if (rules.life) {                                      //// çàäàíèå íàïðàâëåíèÿ äâèæåíèÿ çìåéêè
		...
		if ((!rules.life) && (rules.start)) {
			...
			textEndScore.setString("Âàø ðåçóëüòàò: " + playerScoreString.str());
			textEndScore.setPosition(300, 225);
			window.draw(textEndScore);
			textRestart.setString("Äëÿ ðåñòàðòà íàæìèòå «R»");
			textRestart.setPosition(220, 325);
			window.draw(textRestart);
			textexit.setString("Âûõîä «Escape»");
			textexit.setPosition(315, 425);
			window.draw(textexit);
		}

		if (!rules.start) {
			textstart.setString("Íà÷àòü èãðó «Tab»");
